    ...
    <a-form>
      <a-form-item
        ...
        <a-tooltip placement="topLeft" >
          <template slot="title">
            <span>您隶属于多个区域，请选择当前所在区域</span>
          </template>
          <a-avatar style="backgroundColor:#87d068" icon="gold" />
        </a-tooltip>
        <a-select v-model="departSelected" :class="{'valid-error':validate_status=='error'}" placeholder="请选择登区域" style="margin-left:10px;width: 80%">
          ...
          getAction(url).then(res=>{
            if(res.success){
              ...
              if(this.currDepartName){
                this.currTitle ="区域切换（当前区域 : "+this.currDepartName+"）"
              }
      ...
      show(){
        ...
        this.loadDepartList().then(()=>{
          ...
          if(!this.departList || this.departList.length<=0){
            this.$message.warning("您尚未设置区域信息!")
            this.departClear()
          }
        })
      },
