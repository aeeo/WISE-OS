        ...
        <a-tab-pane tab="基本信息" key="1" >
          <a-card :bordered="false" v-if="selectedKeys.length>0">
            <a-form-model ref="form" :model="model" :rules="validatorRules">
              <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" prop="departName" label="区域名称">
                <a-input placeholder="请输入区域名称" v-model="model.departName" />
              </a-form-model-item>
              <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="父区域">
                <a-tree-select
                  style="width:100%"
  ...
  export default {
    ...
    data() {
      return {
        ...
        validatorRules: {
          departName: [{required: true, message: '请输入区域名称!'}],
          orgCode: [{required: true, message: '请输入区域编码!'}],
          orgCategory:[{required: true, message: '请输入区域类型!'}],
    ...
    methods: {
      ...
      handleAdd(num) {
        if (num == 1) {
          ...
          if (!key) {
            this.$message.warning('请先点击选中父区域！')
            return false
          }
